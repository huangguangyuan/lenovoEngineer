    ...
    confirmFun(){
        ...
        _this.$http.post(reqUrl,data).then(res => {
            if(res.data.code == 200){
                _this.$dialog.alert({message:'提交成功~'});
            }
        })
    }
  },
...
.noFinish{
    ...
    .banner{width: 375px;height: 75px;margin: 0 auto;background-color: #ff9600;text-align: center;font-size: 25px;color: #ffffff;line-height: 75px;}
    .describe{
        margin-top: 15px;
    }
    .content-wrap{
        width: 350px;display: flex;justify-content: space-around;margin: 20px auto 0;
        .container{
            width: 140px;height: 140px;box-sizing: border-box;border-radius: 8px;background-color: #f7f7f7;border:1px #e1e1e1 solid;margin-top: 10px;position: relative;display: flex;justify-content: center;align-items: center;color: #e0e0e0;text-align: center;margin-bottom: 10px;font-size: 14px;
            img{position: absolute;width: 100%;height: 100%;top:0;left: 0;border-radius: 8px;}
        }
    }
    .btn-wrap{width: 350px;margin: 30px auto 20px;}
    .success-mask {
      width: 80%;
      padding: 40px 10px;
      display: flex;
